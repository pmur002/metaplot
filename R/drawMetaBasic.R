drawMetaBasic <-
function(matrix, plotCol, plotHead, xlab, refLine, plotWidth,
         plotPar, xlog, xticks, boxSize, align, clip, newpage, fit, 
         abbreviate, vpName, ... 
         ){
  ...
  if (rownames(labeltext[1,,drop=F]) == "title") {
    ...
    for(i in 1:nr){
      if (!is.null(labels[[j]][[i]])){
        ...
        pushViewport(viewport(layout.pos.row=i,layout.pos.col=2*j-1,
                              name=paste(labels[[j]][[i]]$name, i, j, sep=".")))
        else             
        pushViewport(viewport(layout.pos.row=i,layout.pos.col=2*(j+1)-1,
                              name=paste(labels[[j]][[i]]$name, i, j, sep=".")))
  ...
  if (xlog){
     ...
     if (length(ticks)){  
         ...
         xax1<-editGrob(xax, gPath("labels"),  
                        label=sprintf("%g",as.numeric(format(ticks,digits=2))))
  ...
  # draw plot heading
  grid.text(name="plotHead", plotHead, y=unit((nr-0.5)*lineScale, "lines"), 
  ...
  if(!is.null(xlab)) {
    ...
    grid.text(name="xlab1", xlab[1],          
           x=unit(refLine-0.3, "native"), y=unit(lineScale, "lines"), 
  ...
  for (i in 1:nr) {
    ...
    if (is.summary[i])
     drawSummaryCI(lower[i], mean[i], upper[i], info[i], studynames[i], plotPar)
    else
     drawNormalCI(lower[i], mean[i], upper[i], info[i], studynames[i], plotPar)
