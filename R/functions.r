...
generateTextGrobs <- function(labeltext, align, is.summary, plotPar, 
  ...
  for(j in 1:nc){
    ...
    for(i in 1:nr){
      ...
      if (rownames(labeltext[i,,drop=F]) == "title") {
        ## set up gp for title
        titlegp <- plotPar$title
        titlegp$cex <- scale*titlegp$cex             
        gp <- do.call("gpar", titlegp)
        studynames[i] <- "title"
      } else if (rownames(labeltext[i,,drop=F]) == "subtitle") {
      ...
      } else if (rownames(labeltext[i,,drop=F]) == "hetero") {
...
{  
    ...
    if (1 == plotCol)  {colWidth <- unit.c(plotWidth, colgap)
      for(i in 2:(nc+1)) {                                                    
        colWidth <- unit.c(colWidth, max(unit(rep(1, sum(rowWidth)),
        "grobwidth", labels[[i - 1]][rowWidth])), colgap)
      }                 
    } 
    else {colWidth <- unit.c(max(unit(rep(1, sum(rowWidth)), "grobwidth",
